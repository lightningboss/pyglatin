...
def isItAWord(word):
    if len(word) > 0 and word.isalpha(): # Check for common mistakes in writing
        ...
        return 'Error: You didn\' enter a word! (Error Code: 0x589dfff)'

def toEng():
    original = getInput()
    if not (original[len(original)-2:len(original)] == suffix):
...
def toPyg():
    original = getInput()
    if (original[len(original)-2:len(original)] == suffix):
